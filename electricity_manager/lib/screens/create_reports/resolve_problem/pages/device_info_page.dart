...
class ResolveDevicePage extends StatefulWidget {
  const ResolveDevicePage(
      {Key? key,
  ...
  @override
  _ResolveDevicePageState createState() => _ResolveDevicePageState();
}

class _ResolveDevicePageState extends State<ResolveDevicePage> {
  final _formKey = GlobalKey<FormState>();
  final _deviceActions = [
  ...
  void _next() {
    if (_formKey.currentState != null && _formKey.currentState!.validate()) {
      ...
      final devices = _deviceWidgets.map((e) {
        ...
        e.device?.state = e.stateCtrl.text.trim();
        return e.device!;
      }).toList();
  ...
  Widget _buildDeviceItem(int index, DeviceWidgetModel deviceModel) {
    ...
    final deviceActionSelected = _deviceActions.firstWhere(
        (element) => _deviceWidgets[index].device?.action == element.id,
        orElse: () => DeviceAction.returnAction());
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: AppField(
                controller: deviceModel.stateCtrl,
                label: 'Trạng thái',
            ...
            Expanded(
              child: AppField(
                controller: deviceModel.actionCtrl,
                label: 'Hành động',
                enable: false,
                textInputAction: TextInputAction.done,
                hintText: 'Nhập trạng thái thiết bị',
